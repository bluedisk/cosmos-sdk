...
func TestCoinSend(t *testing.T) {
	...
	require.Equal(t, "luna", mycoins.Denom)
...
func TestBonding(t *testing.T) {
	name, password, denom := "test", "1234567890", "luna"
	...
	acc = getAccount(t, port, addr)
	coins = acc.GetCoins()
	require.Equal(t, int64(40), coins.AmountOf("luna").Int64())
...
func TestDeposit(t *testing.T) {
	...
	// query proposal
	proposal = getProposal(t, port, proposalID)
	require.True(t, proposal.GetTotalDeposit().IsEqual(sdk.Coins{sdk.NewInt64Coin("luna", 10)}))

	// query deposit
	deposit := getDeposit(t, port, proposalID, addr)
	require.True(t, deposit.Amount.IsEqual(sdk.Coins{sdk.NewInt64Coin("luna", 10)}))
}
...
func doSendWithGas(t *testing.T, port, seed, name, password string, addr sdk.AccAddress, gas string, gasAdjustment float64, queryStr string) (res *http.Response, body string, receiveAddr sdk.AccAddress) {
	...
	chainID := viper.GetString(client.FlagChainID)
	// send
	coinbz, err := cdc.MarshalJSON(sdk.NewInt64Coin("luna", 1))
	if err != nil {
		panic(err)
	}
...
func doIBCTransfer(t *testing.T, port, seed, name, password string, addr sdk.AccAddress) (resultTx ctypes.ResultBroadcastTxCommit) {
	...
	}`, "luna", name, password, chainID, accnum, sequence))
...
func doDelegate(t *testing.T, port, seed, name, password string,
	...
	}`, delAddr, valAddr, "luna", amount, name, password, chainID, accnum, sequence))
...
func doSubmitProposal(t *testing.T, port, seed, name, password string, proposerAddr sdk.AccAddress, amount int64) (resultTx ctypes.ResultBroadcastTxCommit) {
	...
	jsonStr := []byte(fmt.Sprintf(`{
		...
		"proposal_type": "Text",
		"proposer": "%s",
		"initial_deposit": [{ "denom": "luna", "amount": "%d" }],
		"base_req": {
			"name": "%s",
			"password": "%s",
			"chain_id": "%s",
